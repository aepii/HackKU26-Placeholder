  ...
  if (items.length === 0) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.emptySubtitle}>Analyze an architecture diagram to see it here.</Text>
      </View>
    );
  }
        ...
        <TouchableOpacity style={styles.card} activeOpacity={0.85} onPress={() => handleOpen(item)}>
          ...
          <View style={styles.info}>
            <Text style={styles.nodeCount}>
              {item.nodes.length} node{item.nodes.length !== 1 ? "s" : ""} · {item.edges.length} edge{item.edges.length !== 1 ? "s" : ""}
            </Text>
            <Text style={styles.preview} numberOfLines={2}>
              {item.feedback[0] ?? "No feedback"}
            </Text>
            <View style={styles.tags}>
              {Array.from(new Set(item.nodes.map(n => n.type))).slice(0, 4).map(t => (
...
const styles = StyleSheet.create({
  ...
  tag: { backgroundColor: "#f1f5f9", borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2 },
